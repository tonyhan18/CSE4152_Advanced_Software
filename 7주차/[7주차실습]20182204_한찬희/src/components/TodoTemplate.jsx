...
function TodoTemplate() {
  ...
  // Deleted List에 추가한 후에, 기존 Done List에서 제외
  const onRemoveDoneTodo = (id) => {
    // 7주차 실습 구현
    // id에 해당하는 항목을 todos 배열에서 찾음
    const doneTo = doneTodos.find((todo) => todo.id === id);
    setDeletedTodos([...deletedTodos, doneTo]);
    // filter 함수 사용해서 id에 해당하는 항목만 todos 배열에서 제외
    setDoneTodos(doneTodos.filter((todo) => todo.id !== id));
  };

  // 주어진 What To Do 항목(체크안된 항목)을 찾아 항목 객체의 속성(키)을 사용하여
  // 체크한 후, Done List에 추가하고 What To Do에서 제외
  ...
  // What To Do에 추가하고 Done List에서 제외
