...
void practice3_8(void) {
	...
	printf("\n>>>>>>>>>> practice3_8 <<<<<<<<<\n");
	double ans = 0.0;
	printf("초기값 1 : x(0) = (0.1, 0.1, -0.1)\n");
	fprintf(fp_w, "초기값 1 : x(0) = (0.1, 0.1, -0.1)\n");
	x[0] = 0.1;
	x[1] = 0.1;
	x[2] = -0.1;
	...
	ans = 3.0 * x[0] - cos(x[1] * x[2]) - 0.5;
	printf("|f1(x)| = %lf\n", fabs(ans));
	fprintf(fp_w, "|f1(x)| = %lf\n", fabs(ans));
	ans = x[0] * x[0] - 81.0 * (x[1] + 0.1) * (x[1] + 0.1) + sin(x[2]) + 1.06;
	printf("|f2(x)| = %lf\n", fabs(ans));
	fprintf(fp_w, "|f2(x)| = %lf\n", fabs(ans));
	ans = exp(-x[0] * x[1]) + 20.0 * x[2] + (10 * M_PI - 3.0) / 3.0;
	printf("|f3(x)| = %lf\n", fabs(ans));
