...
__global__ void Fibonacci_Kernel(int* x, int* y)
{
    ...
    y[tid] = (int)((tmp0 - tmp1) / sqrt_5 + 0.5f);
}
...
cudaError_t Fibonacci_GPU(int* h_Fibo_n, int* h_Fibo_v, int* h_Fibo_v_GPU)
{
    ...
    CUDA_CALL(cudaMalloc(&d_FiboN, size))

        CUDA_CALL(cudaMemcpy(d_FiboN, h_Fibo_n, size, cudaMemcpyHostToDevice))


        CUDA_CALL(cudaMalloc(&d_FiboV, size))
    ...
    Fibonacci_Kernel << < dimGrid, dimBlock >> > (d_FiboN, d_FiboV);
    ...
    CUDA_CALL(cudaGetLastError())
        ...
        CUDA_CALL(cudaMemcpy(h_Fibo_v_GPU, d_FiboV, size, cudaMemcpyDeviceToHost))
