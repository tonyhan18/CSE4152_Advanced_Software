...
__global__ void Sum_n_elements_Kernel(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
    ...
    d_SumOfArrayElements[id] = 0;
    for (int k = id - Nf; k <= id + Nf; ++k)
    {
        if (k < 0 || k >= N) continue;
        d_SumOfArrayElements[id] += d_ArrayElements[k];
    }
}
...
cudaError_t Sum_n_elements_GPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_GPU, int Nf) {
    ...
    size = N * sizeof(int);
    CUDA_CALL(cudaMalloc(&d_Arr, size))

        CUDA_CALL(cudaMemcpy(d_Arr, p_ArrayElements, size, cudaMemcpyHostToDevice))
    ...
    s
    Sum_n_elements_Kernel << < dimGrid, dimBlock >> > (d_Arr, d_Sum, N, Nf);
